...
from .modeling_bert import BaseModel, BertEmbeddings, BertEncoder, BertCLS, BertCLSMulti, SurvTraceTimeDecoderHead
...
class SurvTraceSingle(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.duration_index = config['duration_index']
        self.use_gpu = False
        self.decoder_head = SurvTraceTimeDecoderHead(config)
        self.time_queries_tensor = torch.arange(4).long()  # exclude start/end pad
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            seq_length = input_ids.size(1) + input_nums.size(1)  # ✅ full sequence length
        elif inputs_embeds is not None:
            input_shape = inputs_embeds.size()[:-1]
            batch_size, seq_length = input_shape
        else:
        ...
        all_row_attentions = encoder_outputs[1] if output_attentions else None
        all_col_attentions = encoder_outputs[2] if output_col_attentions else None
        
        predict_logits = self.cls(encoder_outputs[0])

        if output_attentions:
          return sequence_output, predict_logits, all_row_attentions, all_col_attentions
        else:
          return sequence_output, predict_logits

    def predict(self, x_input, batch_size=None, attention_mask=None):
        ...
        if attention_mask is not None and not isinstance(attention_mask, torch.Tensor):
          attention_mask = torch.tensor(attention_mask.values).float()
            ...
            if batch_size is None:
                ...
                for idx in range(num_batch):
                    ...
                    if attention_mask is not None:
                        ...
                        batch_mask = None
                    batch_pred = self.forward(batch_x_cat,batch_x_num, attention_mask=batch_mask)
                    preds.append(batch_pred[1])
                preds = torch.cat(preds, dim=0) 
        return preds

    def predict_hazard(self, input_ids, batch_size=None, attention_mask=None):
        preds = self.predict(input_ids, batch_size, attention_mask=attention_mask)
        hazard = F.softplus(preds)
        hazard = pad_col(hazard, where="start")
        return hazard

    def predict_risk(self, input_ids, batch_size=None, attention_mask=None):
        surv = self.predict_surv(input_ids, batch_size, attention_mask=attention_mask)
        return 1 - surv

    def predict_surv(self, input_ids, batch_size=None, attention_mask=None, epsilon=1e-7):
        hazard = self.predict_hazard(input_ids, batch_size, attention_mask=attention_mask)
        surv = hazard.cumsum(1).mul(-1).exp()
        return surv

    def predict_surv_df(self, input_ids, batch_size=None, attention_mask=None):
        surv = self.predict_surv(input_ids, batch_size, attention_mask=attention_mask)
        return pd.DataFrame(surv.to("cpu").numpy().T, self.duration_index)
