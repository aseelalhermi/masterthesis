...
class Trainer:
    ...
    def train_single_event(self,
        ...
        **kwargs,
        ...
        df_train, df_y_train, attention_mask_train = train_set
        durations_train, events_train = self.get_target(df_y_train)
        attention_mask_train = attention_mask_train[df_train.columns]

        if val_set is not None:
            df_val, df_y_val, attention_mask_val = val_set
            ...
            attention_mask_val = attention_mask_val[df_val.columns]
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(num_train_batch):
                ...
                batch_mask = tensor_mask[batch_idx*batch_size:(batch_idx+1)*batch_size]

                outputs = self.model(
                    input_ids=batch_x_cat,
                    input_nums=batch_x_num,
                    attention_mask=batch_mask,
                    output_attentions=False,
                )
                if len(outputs) == 4:
                    _, phi, row_probs, col_probs = outputs

                else:
                    _, phi = outputs

                if len(self.metrics) == 1: # only NLLPCHazardLoss is asigned
                    batch_loss = self.metrics[0](phi, batch_y_train[:,0].long(), batch_y_train[:,1].long(), batch_y_train[:,2].float(), reduction="mean")
                else:
            ...
            if val_set is not None:
                self.model.eval()
                with torch.no_grad():
                  tensor_mask_val = torch.tensor(attention_mask_val.values).float()
                  if self.use_gpu:
                      tensor_mask_val = tensor_mask_val.cuda()

                  phi_val = self.model.predict(tensor_val, val_batch_size, attention_mask=tensor_mask_val)
